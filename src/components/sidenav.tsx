...
export const SideNav = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <nav
      className={`h-full ${isCollapsed ? 'w-20' : 'w-80'} fixed left-0 top-0 bg-white shadow-lg flex flex-col px-4 py-6 gap-4 transition-all duration-300`}
    >
      ...
      <div className="mb-2 flex h-14 items-center gap-4">
        <img
          src={GymLogo}
          className={`h-full w-auto ${isCollapsed ? 'hidden' : 'block'}`}
        />
      ...
      <div className="flex flex-col gap-2">
        <Link
          to={PRIVATE_LINK_ROUTES.ROUTINES}
          className="flex items-center gap-2 text-gray-700"
        >
          <span className="material-symbols-outlined">exercise</span>
          {!isCollapsed && 'Mis Rutinas'}
        </Link>
        <Link
          to={PRIVATE_LINK_ROUTES.PROGRESS}
          className="flex items-center gap-2 text-gray-700"
        >
        ...
        >
          ...
          {!isCollapsed && 'Monitoreo'}
        </Link>
        <Link
          to={PRIVATE_LINK_ROUTES.MACHINES}
          className="flex items-center gap-2 text-gray-700"
        >
          <span className="material-symbols-outlined">construction</span>
          {!isCollapsed && 'Administrar equipos'}
        </Link>
      </div>
      ...
      {role === 'admin' && (
        <div className="mt-4">
          <span
            className={`text-sm font-bold text-gray-600 ${isCollapsed ? 'hidden' : 'block'}`}
          >
            Administración
          </span>
