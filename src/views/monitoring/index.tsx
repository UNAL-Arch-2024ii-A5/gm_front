...
export const Monitoring = () => {
  ...
  const [createEvent, { data: mutationEvent, loading: loadingMutation, error: errorMutation }] = useMutation(CREATE_EVENT);
  ...
  return (
    <div>
      ...
      {data && <table className='my-5'>
        {data.allMonitoringEvents.map((item: NewEvent) => <tr><td>{item.name}</td><td>{item.machineID}</td><td>{item.userID}</td></tr>)}
      </table>}
      <button onClick={refetch}>Re cargar eventos</button>
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4 my-4 px-4'>
        ...
        <label className='w-1/3 flex-col flex'>
          Nombre del evento
          <input type='text' placeholder='Nombre del evento' {...register("name")} className='bg-neutral-50 rounded-sm mt-1'/>

        </label>
        <label className='w-1/2 flex-col flex'>
          MachineID
          <input type='text' readOnly defaultValue={"6348acd2e1a47ca32e79f46f"} placeholder='MachineID' {...register("machineID")} className='bg-neutral-50 rounded-sm mt-1'/>
        </label>
        <label className='w-1/2 flex-col flex'>
          UserID
          <input type='text' readOnly defaultValue={"6348acd2e1a47ca32e79f46f"} placeholder='UserID' {...register("userID")} className='bg-neutral-50 rounded-sm mt-1'/>

        </label>
        </div>
        <button className='self-end border-1 border-neutral-950 bg-neutral-100 px-3 py-2 rounded-sm'>Crear evento</button>
      </form>
